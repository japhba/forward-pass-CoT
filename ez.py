...
def easy_generate(model: t.nn.Module, tokenizer: tr.AutoTokenizer, prompts: list[str], output_completion_only=False, **kwargs):
    """
    Args:
        model: The model to generate from
        prompts: A list of prompts to generate from
        output_completion_only: Whether to return only the newly generated completion tokens, excluding the input prompts.
    ...
    if output_completion_only:
        # Exclude the input prompts and return only the newly generated tokens
        input_lengths = [len(tokenizer.decode(ids, skip_special_tokens=True)) for ids in inputs.input_ids]
        decoded = [d[length:] for d, length in zip(decoded, input_lengths)]
    return decoded
